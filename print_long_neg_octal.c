...
/**
 * print_long_neg_octal - Prints a negative number in octal
 ...
 */
int print_long_neg_octal(long num, char *buffer, int *len_ptr)
{
	int pow = 0;
	int len = 0;
	int i, allocated = 0;
	...
	while (num > calc_long_power(8, pow))
		pow++;
	num = calc_long_power(8, pow) - num;
	temp = num;
	while (temp > 0)
	{
		temp /= 8;
	...
	{
		allocated += 1;
		add_to_buffer('1', buffer, len_ptr);
		for (i = 0; i < (21 - len); i++)
		{
